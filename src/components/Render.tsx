...
export default function Render() {
  ...
  const [frameMetrics,setFrameMetrics]=useState<{drawCalls:number;frameMs:number}>();
  ...
  return <main className="page"><a href="#/" className="eyebrow">Hats which look like space</a><h1>Charting your sky</h1>
    {errors.length ? <><p role="alert">{errors.map(e=>e.message).join(" ")}</p><Button onClick={()=>navigate(`/design?${query}`)}>Edit design</Button></> : <>
      <div className="hat-canvas"><HatCanvas stitches={stitches} setStitches={cached ? undefined : setStitches} sky={sky} setSky={setSky} orientationParameters={design.orientation} onReady={()=>setReady(true)} onMetrics={setMetrics} onFrameMetrics={params.has("diagnostics")?setFrameMetrics:undefined} simulationActive={active} setSimulationActive={cached ? undefined : setActive}/></div>
      ...
      {params.has("diagnostics") && <output className="diagnostics">{JSON.stringify({settle:metrics,frames:frameMetrics})}</output>}
