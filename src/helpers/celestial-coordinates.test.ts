...
describe("daysInMonth", () => {
  ...
  it("knows the thirty-day months", () => {
    for (const month of [4, 6, 9, 11])
      expect(daysInMonth(month, 2025)).toBe(30);
    for (const month of [1, 3, 5, 7, 8, 10, 12])
      expect(daysInMonth(month, 2025)).toBe(31);
  });
});
...
describe("zenithFor", () => {
  it("has the observer's latitude directly overhead", () => {
    expect(
      zenithFor(
        { year: 2025, month: 6, day: 1, hour: 0, minute: 0 },
  ...
  it("puts RA 0h overhead at Greenwich midnight on the September equinox", () => {
    const { ra } = zenithFor(
      { year: 2025, month: 9, day: 22, hour: 0, minute: 0 },
  ...
  it("puts RA 12h overhead at Greenwich midnight on the March equinox", () => {
    const { ra } = zenithFor(
      { year: 2025, month: 3, day: 20, hour: 0, minute: 0 },
      { latitude: 51.5, longitude: 0 },
    );
  ...
  it("always answers inside [0, 24)", () => {
    for (const longitude of [-180, -179.9, -90, 0, 90, 179.9, 180]) {
      for (const hour of [0, 6, 12, 23]) {
        const { ra } = zenithFor(
          { year: 2025, month: 12, day: 31, hour, minute: 59 },
  ...
  it("defaults the year to this one", () => {
    ...
    const withYear = zenithFor(
      { year, month: 5, day: 5, hour: 5, minute: 5 },
      { latitude: 0, longitude: 0 },
    );
    const without = zenithFor(
      { month: 5, day: 5, hour: 5, minute: 5 },
      { latitude: 0, longitude: 0 },
    );
...
describe("right ascension and catalogue longitude", () => {
  ...
  it("round trips", () => {
    for (const hours of [0, 0.5, 3, 6, 11.9, 12.1, 18, 23.9]) {
      expect(longitudeToRaHours(raHoursToLongitude(hours))).toBeCloseTo(
        hours,
        9,
      );
    }
