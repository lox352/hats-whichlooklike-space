...
export function cacheHat(design: HatDesign, stitches: Stitch[], sky: SkyMarks) {
  try {
    sessionStorage.setItem(
      key,
      JSON.stringify({ key: designKey(design), stitches, sky }),
    );
  } catch {
    /* regenerable */
  }
...
export function readHat(design: HatDesign): CachedHat | undefined {
  try {
    ...
    if (
      value?.key === designKey(design) &&
      Array.isArray(value.stitches) &&
      value.stitches.length &&
      Array.isArray(value.sky?.stars) &&
      Array.isArray(value.sky?.constellations)
    )
      return value;
  } catch {
    /* invalid cache */
